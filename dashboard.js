(function () {
    ...
    var MAX_PER_BLOCK = 5;
    var BLOCK_IDS = ["info-search", "financial-security", "egov", "network-culture", "device-care"];
    ...
    var BLOCK_META = {
        ...
        "financial-security": { title: "Қаржылық қауіпсіздік", icon: "🛡️", color: "#f57c00" },
        egov: { title: "eGov қызметтері", icon: "🏛️", color: "#5e35b1" },
        "network-culture": { title: "Желі мәдениеті", icon: "💬", color: "#c62828" },
    ...
    function renderOverallProgress(aggregates, results) {
        ...
        if (blocksText) {
            blocksText.textContent = aggregates.completedCount + " / " + TOTAL_BLOCKS + " блок аяқталды";
        }
    ...
    function renderMotivation(aggregates) {
        ...
        var message = "🚀 Бастайық! Алғашқы тестті тапсырып, деңгейіңізді анықтаңыз.";

        if (completed === 0) {
            message = "🚀 Бастайық! Алғашқы тестті тапсырып, деңгейіңізді анықтаңыз.";
        } else if (completed <= 2) {
            message = "💪 Жақсы бастама! Жалғастырыңыз — тағы бірнеше блок сізді күтіп тұр.";
        } else if (completed <= 4) {
            var missing = 18 - score;
            if (missing < 0) missing = 0;
            if (missing === 0) {
                message = "🔥 Озат деңгейіне өте жақынсыз! Тағы бір блокты аяқтап көріңіз.";
            } else {
    ...
    function renderActivity(results) {
        ...
        if (lastThree.length === 0) {
            container.innerHTML = '<p class="activity-empty">Әлі тест тапсырылмаған. Бастаңыз! →</p>';
            return;
        }
        ...
        lastThree.forEach(function (item) {
            ...
            icon.style.background = "linear-gradient(135deg, " + item.color + "55, " + item.color + "AA)";
    ...
    function initDigcompAccordion() {
        ...
        items.forEach(function (item, index) {
            ...
            header.addEventListener("click", function () {
                ...
                if (isActive) {
                    // Егер әрқашан біреуі ашық болсын десеңіз, мынаны қалдырып, жабуды өшірмей қоюға болады.
                    closeItem(item);
                } else {
                    openItem(item);
                }
            ...
            // Бірінші элементті әдепкі ашық күйге қою
            if (index === 0) {
                openItem(item);
            }
        });
    ...
    function initAuthAndName() {
        try {
            ...
            // if parsing fails, still show page but skip name
        }
