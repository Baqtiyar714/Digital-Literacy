...
app.get("/admin/analytics", checkAdmin, async (req, res) => {
  try {
    ...
    try {
      const blockCols = [
        { id: "information", col: "information_score" },
        { id: "communication", col: "communication_score" },
        { id: "content", col: "content_score" },
        { id: "safety", col: "safety_score" },
        { id: "problem", col: "problem_score" },
      ];
      const blockRows = [];
      for (const b of blockCols) {
        const r = await pool.query(
          `SELECT '${b.id}' AS competency, ROUND(AVG(${b.col}::float/20*100)) AS avg_pct, COUNT(*) AS count FROM test_results WHERE ${b.col} IS NOT NULL`,
        );
        blockRows.push(r.rows[0]);
      }
      byBlock = { rows: blockRows };
    } catch (e) {
